...
const MichaelWing = () => {
  ...
  const fetchArtworks = async () => {
    try {
      window.currentOpenPanel?.();
      window.currentOpenPanel = () => setShowPanel(false);
  ...
  useEffect(() => {
    return () => {
      if (window.currentOpenPanel === setShowPanel) {
    ...
    <>
      ...
      {showPanel && (
        ...
        >
          ...
          <h2 style={{ color: '#0dcaf0', marginTop: 0 }}>Michael C. Rockefeller Wing</h2>
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {artworks.map((art) => (
              <li key={art.objectID} style={{ marginBottom: '1rem' }}>
                <div
                  onClick={async () => {
                    const res = await fetch(`http://localhost:5000/artworks/${art.objectID}`);
                    const data = await res.json();
